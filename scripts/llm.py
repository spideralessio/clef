...
ANTHROPIC_PREFIXES: Final[tuple[str, str,
...
class LLM(BaseLLM):
    ...
    def _prepare_completion_params(
        ...
        params = {
            ...
            "stop":
            self.stop,
            "max_tokens" if self.max_tokens else "max_completion_tokens":
    ...
    def _handle_streaming_response(
        ...
        accumulated_tool_args: defaultdict[
            int, AccumulatedToolArgs] = defaultdict(AccumulatedToolArgs)
        ...
        try:
                ...
                try:
                    ...
                    if choices and len(choices) > 0:
                        ...
                        if delta:
                            ...
                            if chunk_content is None and isinstance(
                                    delta, dict):
                    ...
                    logging.debug(
                        f"Chunk format: {type(chunk)}, content: {chunk}")
            ...
            if not full_response.strip() and chunk_count == 0:
                ...
                non_streaming_params.pop(
                    "stream_options",
                    None)  # Remove stream_options for non-streaming call
            ...
            if not full_response.strip() and chunk_count > 0:
                ...
                if last_chunk is not None:
                    try:
                        ...
                        choices = None
                        if isinstance(last_chunk,
                                      dict) and "choices" in last_chunk:
                        ...
                        logging.debug(
                            f"Error extracting content from last chunk: {e}")
                        logging.debug(
            ...
            try:
                if last_chunk:
                    ...
                    if choices and len(choices) > 0:
                        ...
                        if message:
                            if isinstance(message,
                                          dict) and "tool_calls" in message:
            ...
            tool_result = self._handle_tool_call(tool_calls,
                                                 available_functions)
            ...
            if full_response.strip():
                logging.warning(
                    f"Returning partial response despite error: {e!s}")
                self._handle_emit_call_events(
                    response=full_response,
                    call_type=LLMCallType.LLM_CALL,
                    from_task=from_task,
            ...
            if not hasattr(crewai_event_bus, "emit"):
                raise AttributeError(
                    "crewai_event_bus must have an 'emit' method") from e
            crewai_event_bus.emit(
                self,
    ...
    def _handle_streaming_tool_calls(
        ...
        for tool_call in tool_calls:
            ...
            if (current_tool_accumulator.function.name
    ...
    def _handle_tool_call(
        ...
        function_args = {
        }  # Initialize to empty dict to avoid unbound variable
        ...
        if function_name in available_functions:
            try:
                ...
                fn = available_functions.get(
                    function_name,
                    lambda: None)  # Ensure fn is always a callable
                logging.error(
            ...
            try:
                ...
                if unsupported_stop:
                    if ("additional_drop_params" in self.additional_params
                            and isinstance(
                                self.
                                additional_params["additional_drop_params"],
                                list)):
                        self.additional_params[
                            "additional_drop_params"].append("stop")
                    else:
                        self.additional_params = {
    ...
    def _format_messages_for_provider(
        ...
        if ("ollama" in self.model.lower() and messages
    ...
    def get_context_window_size(self) -> int:
        ...
        self.context_window_size = int(DEFAULT_CONTEXT_WINDOW_SIZE *
                                       CONTEXT_WINDOW_USAGE_RATIO)
        for key, value in LLM_CONTEXT_WINDOW_SIZES.items():
            if self.model.startswith(key):
                self.context_window_size = int(value *
                                               CONTEXT_WINDOW_USAGE_RATIO)
        return self.context_window_size
    ...
    def set_env_callbacks() -> None:
        ...
        with suppress_warnings():
            ...
            if success_callbacks_str:
                success_callbacks = [
                    cb.strip() for cb in success_callbacks_str.split(",")
                    if cb.strip()
                ]
